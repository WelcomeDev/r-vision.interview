...
Вы дорабатываете консоль управления сканированием активов. Нужен мастер (wizard) из четырёх
...
| Поле | Контрол | Правила |
| --- | --- | --- |
| Периодичность | `Select` (`api.getPeriodicity`) | обязательная |
| Время запуска | `TextInput` | формат `ЧЧ:ММ`; обязательно, если периодичность ≠ «Однократно» |
| Получатели уведомлений | `MultiSelect` (`api.getUsers`) | минимум один |
| Только критичные находки | `Checkbox` | без валидации |

### Шаг 4. Подтверждение

- Сводка по всем шагам в человекочитаемом виде (названия, а не id: тип, профиль, теги,
  хосты активов с портами, получатели).
...
4. Состояния загрузки и ошибок сети видны пользователю. Обёртку над загрузкой данных
...
**Дальше по времени:** шаг 3, сводка и сабмит, разбор 422 с переходом на нужный шаг,
...
| 45–60 | Шаг 3–4, сабмит и ошибки бэкенда |
| 60–75 | Обсуждение решения |
